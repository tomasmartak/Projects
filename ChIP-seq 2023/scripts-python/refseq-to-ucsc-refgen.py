...
"""Description: looks through your target reference genome (refgen.fasta) and replaces RefSeq annotations with
...
def parse_assembly_report(report_url):
    ...
    matcher = matcher[['RefSeq-Accn', 'UCSC-style-name']]
...
def read_genome(refgen_path):
    genome = {}
    with open(refgen_path, 'r', encoding='ISO-8859-1') as infile:
        seq_id = None
        seq = []
        for line in infile:
            line = line.strip()
                ...
                if line.startswith('>'):
                    if seq_id is not None:
                        ...
                        for i, row in matcher.iterrows():
                            new_seq_id = new_seq_id.replace(row['RefSeq-Accn'], row['UCSC-style-name'])
...
def gff_sub(gff_path, output_path, matcher):
    # Load the matcher dataframe into a dictionary to avoid loading it multiple times
    matcher_dict = matcher.set_index('RefSeq-Accn')['UCSC-style-name'].to_dict()

    with gzip.open(gff_path, 'rt') as infile, open(output_path, 'w') as outfile:
        gff_counter = 0
        for line in infile:
            if line.startswith('#'):
                outfile.write(line)
                ...
                seq_id = matcher_dict.get(fields[0], fields[0])
                start, end, strand = fields[3], fields[4], fields[6]
                attributes = fields[8]

                # Apply the substitutions for each row of the matcher dataframe
                for refseq_accn, ucsc_name in matcher_dict.items():
                    seq_id = seq_id.replace(refseq_accn, ucsc_name)

                # Use string formatting to construct the modified line
                modified_line = f"{seq_id}\t{fields[1]}\t{fields[2]}\t{start}\t{end}\t{fields[5]}\t{strand}\t{fields[7]}\t{attributes}\n"
...
if __name__ == '__main__':
    ...
    if sys.argv[1].endswith('.gff.gz'):
        ...
        assert refgen_path.endswith('.fasta') or refgen_path.endswith('.fna.gz'), f"{refgen_path} is not in .fasta/.fna/.fna.gz/.gff.gz format. Either change it or this script."
        gff_path = None
